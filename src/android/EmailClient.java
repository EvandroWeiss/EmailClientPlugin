...
public class EmailClient extends CordovaPlugin {
    ...
    public boolean execute(String action, JSONArray args, final CallbackContext callbackContext) {
        try {
            ...
            } else if (EmailClient.ACTION_ADD_ATTACHMENT.equals(action)) {
    ...
    private void getConfig(JSONArray args, CallbackContext callbackContext) throws Exception {
        ...
        } else if("teste".equals(configToGet)){
            callbackContext.sucess(Environment.getExternalStoreDirectory());
        }else{
            callbackContext.error("Email config '" + configToGet + "' does not exists.");
        }
    }

    private void addAttachment(final JSONArray args, final CallbackContext callbackContext) throws Exception {
      mail.addAttachment(args.getString(0), args.getString(1));
      callbackContext.success();
    }
